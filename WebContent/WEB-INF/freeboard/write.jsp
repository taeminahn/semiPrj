	...
	<c:if test="${not empty userId}">
		<div class="mini-navbar" style="text-align: right;">
			<span>${userId}님 환영합니다.</span>&nbsp;|&nbsp; <span><a
				class="mini-item" href="/mypage/view">마이페이지</a>&nbsp;|&nbsp; <a
				class="mini-item" onclick="return confirm('로그아웃 하시겠습니까?')" href="/logout">로그아웃</a></span>
		</div>
	</c:if>
	<!-- navbar -->
	<nav
	...
	<div class="container" id="main">
		<form action="/freeboard/write" method="POST">
					...
					<tr>
						...
						<td colspan="2">
							...
							<c:if test="${bv != null}">
								<input type="text" class="form-control"placeholder="글 제목" name="bbsTitle" maxlength="50"
								value="${bv.boardTitle}"	>
								<input type="hidden" name="num" value="${bv.boardNum}">
								<input type="hidden" name="bid" value="${bv.boardId}">
							</c:if>
						</td>
					</tr>
				</thead>
				<tbody>
					<tr class="text-center">
						<td colspan="2">
							...
							<c:if test="${bv != null}">
								<textarea class="form-control"placeholder="글 내용" name="bbsContent" maxlength="2048"style="height: 350px;">${bv.boardContent}</textarea>
							</c:if>
						</td>
					</tr>
				</tbody>
			...
			<c:if test="${bv == null}">
				...
				class="btn btn-primary float-right" value="취소">
				<input type="submit" onclick="alert('글이 등록되었습니다.')"
				class="btn btn-primary float-right" value="등록">
			</c:if>
			<c:if test="${bv != null}">
			<input type="submit" onclick="history.back()"
				class="btn btn-primary float-right" value="취소">
				<input type="submit" onclick="alert('글이 수정되었습니다.')"
				class="btn btn-primary float-right" value="수정">
			</c:if>
